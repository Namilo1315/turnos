...
const fmtFullDate = (iso)=> {
  try{
    ...
    const wd = dt.toLocaleDateString('es-AR', { weekday:'short' }).toLowerCase();
...
const c_open=$('#c_open'); const c_close=$('#c_close'); const c_slot=$('#c_slot');
const c_wa=$('#c_wa');     const c_pay=$('#c_pay');     const c_save=$('#c_save');

const svc_list = $('#svc_list'); const svc_add = $('#svc_add');

const publicLink = $('#publicLink'); const copyLink = $('#copyLink'); const qrBox = $('#qr');
...
function isAuthed(){ return sessionStorage.getItem('adminAuth') === 'true'; }
function setAuthed(v){ sessionStorage.setItem('adminAuth', v ? 'true' : 'false'); }
...
function renderKPIs(){
  ...
  let revenue = 0, count=0, cancel=0;
  scoped.forEach(b=>{
    if(b.status==='cancelado'){ cancel++; return; }
    count++;
...
function renderNewBookingForm(){
  a_svc.innerHTML = SERVICES.map(s=>`<option value="${s.id}" data-duration="${s.duration}">${s.name} — ${fmtARS(s.price)}</option>`).join('');
...
function renderAgenda(){
  const mapSvc = Object.fromEntries(SERVICES.map(s=>[s.id, s]));
  const items = filterBookingsByScope(BOOKINGS)
    .sort((a,b)=> (a.date+a.time).localeCompare(b.date+b.time));

  agendaTable.innerHTML = items.map(b=>{
  ...
  agendaTable.querySelectorAll('button[data-act]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      try{
        ...
        Swal.fire({icon:'error', title:'Ups', text:e.message || 'No se pudo completar la acción'});
      }
    });
  });
}
...
clear_filter.addEventListener('click', ()=>{
  filter_date.value='';
  const monthSel = $('#filter_month');
  if(monthSel){ monthSel.value = ymKey(todayISO()); }
  renderKPIs(); renderAgenda(); renderCharts();
});
exportCsv.addEventListener('click', ()=>{
  ...
  scoped.forEach(b=>{
    rows.push([b.date,b.time,(mapSvc[b.serviceId]?.name||''),(b.name||''),(b.phone||''),(b.status||'')]);
  });
...
function ensureMonthSelector(){
  ...
  const wrap = filter_date?.parentElement?.parentElement || filter_date?.parentElement || document;
...
function fillMonthSelector(){
  const sel = $('#filter_month'); if(!sel) return;
  ...
  sel.innerHTML = list.map(ym=> `<option value="${ym}">${label(ym)}</option>`).join('');
...
c_save.addEventListener('click', async ()=>{
  await saveSettings({
    open: c_open.value, close: c_close.value, slot: parseInt(c_slot.value||30,10),
    wa: c_wa.value, pay_alias: c_pay.value, pay: c_pay.value
  });
...
async function updateServiceSafe(id, patch){
  if(typeof DB.updateService === 'function') return await DB.updateService(id, patch);
  if(typeof DB.setService === 'function')    return await DB.setService(id, patch);
  if(typeof DB.listServices === 'function' && typeof DB.saveServices === 'function'){
    const all = await DB.listServices();
...
async function deleteServiceSafe(id){
  if(typeof DB.deleteService === 'function') return await DB.deleteService(id);
  if(typeof DB.removeService === 'function') return await DB.removeService(id);
  if(typeof DB.listServices === 'function' && typeof DB.saveServices === 'function'){
    const all = await DB.listServices();
...
function renderServiceAdminList(){
  svc_list.innerHTML = SERVICES.map(s=>`
    <div class="border rounded p-2 d-flex align-items-center justify-content-between mb-2" data-id="${s.id}">
      <div class="d-flex align-items-center gap-2">
        <span class="d-inline-block rounded-circle" style="width:12px;height:12px;background:${s.color||'#ec4899'}"></span>
  ...
  svc_list.querySelectorAll('button[data-act]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      const row = btn.closest('[data-id]'); const id = row?.dataset.id; if(!id) return;
      const s = SERVICES.find(x=>x.id===id); if(!s) return;

      try{
        if(btn.dataset.act==='del'){
          const ok = await Swal.fire({icon:'warning', title:'¿Borrar servicio?', showCancelButton:true, confirmButtonText:'Borrar', confirmButtonColor:'#ec4899'});
          if(ok.isConfirmed){
            await deleteServiceSafe(id);
            await reloadEverywhere();
            Swal.fire({icon:'success', title:'Servicio borrado', timer:1100, showConfirmButton:false});
          }
        }else{
          const {value: formValues} = await Swal.fire({
            ...
            html: `
              ...
              <input id="svcc" class="swal2-input" placeholder="Color" value="${s.color||'#ec4899'}">
            `,
            focusConfirm: false, confirmButtonText:'Guardar', confirmButtonColor:'#ec4899',
            preConfirm: () => ({
              name: document.getElementById('svcn').value.trim(),
              price: parseInt(document.getElementById('svcp').value||0,10),
              duration: parseInt(document.getElementById('svcd').value||30,10),
  ...
  if(!svc_add.dataset.bound){
    ...
    svc_add.addEventListener('click', async ()=>{
      const {value: formValues} = await Swal.fire({
        ...
        `,
        focusConfirm: false, confirmButtonText:'Agregar', confirmButtonColor:'#ec4899',
        preConfirm: () => {
          const payload = {
      ...
      if(formValues){
        try{
          ...
          await reloadEverywhere();
          Swal.fire({icon:'success', title:'Servicio agregado', timer:1100, showConfirmButton:false});
        }catch(e){
          Swal.fire({icon:'error', title:'Ups', text:e.message || 'No se pudo agregar'});
        }
      }
    });
  }
...
function renderPublicLink(){
  ...
  copyLink.addEventListener('click', ()=>{ navigator.clipboard.writeText(link); copyLink.textContent='Copiado'; setTimeout(()=>copyLink.textContent='Copiar',1200); }, {once:true});
  if(window.QRCode){
    qrBox.innerHTML = '';
    QRCode.toCanvas(document.createElement('canvas'), link, {width: 220}, (err, canvas)=>{
...
async function deleteBookingSafe(id){
  try{
    if(typeof DB.deleteBooking === 'function') return await DB.deleteBooking(id);
    if(typeof DB.removeBooking === 'function') return await DB.removeBooking(id);
    if(typeof DB.delBooking === 'function') return await DB.delBooking(id);
    if(typeof DB.listBookings === 'function' && typeof DB.saveBookings === 'function'){
      const all = await DB.listBookings();
      const next = all.filter(b=> b.id !== id);
      return await DB.saveBookings(next);
    }
  }catch(e){ console.error('deleteBookingSafe:', e); }
  throw new Error('No se pudo borrar el turno');
}
async function updateBookingSafe(id, patch){
  try{
    if(typeof DB.updateBooking === 'function') return await DB.updateBooking(id, patch);
    if(typeof DB.setBooking === 'function')    return await DB.setBooking(id, patch);
    if(typeof DB.listBookings === 'function' && typeof DB.saveBookings === 'function'){
      const all = await DB.listBookings();
      const next = all.map(b=> b.id===id ? {...b, ...patch} : b);
      return await DB.saveBookings(next);
    }
  }catch(e){ console.error('updateBookingSafe:', e); }
...
function renderCharts(){
  ...
  const brand = '#ec4899', soft='rgba(236,72,153,0.12)';
  const rosePalette = ['#ec4899','#f472b6','#fb7185','#fda4af','#f9a8d4','#e879f9','#fca5a5'];
  ...
  const countByDay = days.map(d=> scoped.filter(b=>b.date===d && b.status!=='cancelado').length);
  ...
  const centerTextPlugin = {
    ...
    afterDraw(chart){
      ...
      ctx.font = '900 24px Inter, system-ui, -apple-system, Segoe UI, Roboto';
      ctx.fillStyle = brand;
  ...
  const outerLabelsPlugin = {
    ...
    afterDatasetsDraw(chart){
      ...
      meta.data.forEach((arc, i)=>{
        ...
        x2 = Math.max(chartArea.left + padEdge, Math.min(chartArea.right - padEdge, x2));
        y2 = Math.max(chartArea.top  + padEdge, Math.min(chartArea.bottom - padEdge, y2));

        const alignRight = (x2 >= x);
        const x3 = x2 + (alignRight ? 12 : -12);
        const y3 = y2;

        const label = `${chart.data.labels[i]}: ${chart.data.datasets[0].data[i]}`;
        ctx.textAlign = alignRight ? 'left' : 'right';
        ctx.textBaseline = 'middle';

        const textX = Math.max(chartArea.left + padEdge, Math.min(chartArea.right - padEdge, x3 + (alignRight ? padText : -padText)));
        const textY = Math.max(chartArea.top  + padEdge, Math.min(chartArea.bottom - padEdge, y3));
  ...
  chartDays = new Chart(chartDaysCanvas, {
    type: 'line',
    data: {
      labels: days.map(d=> scope.type==='day' ? d.slice(5).replace('-','/') : d.slice(8,10)),
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: { label: (ctx)=> ` ${ctx.parsed.y} turno(s)` }
        }
      },
      scales: {
        x: { grid:{ display:false }, ticks:{ color:'#6b7280', font:{ family:'Inter' } } },
        y: { beginAtZero:true, grid:{ color:'#ffe4e6' }, ticks:{ stepSize:1, color:'#6b7280', font:{ family:'Inter' } } }
      }
    }
  });

  // Anillo
  const bgColors = svcLabels.map((_, i) => rosePalette[i % rosePalette.length]);
  chartServices = new Chart(chartServicesCanvas, {
    ...
    options: {
      ...
      cutout: '65%',
      layout: { padding: { top: 22, right: 18, bottom: 22, left: 18 } },
      plugins: {
        legend: {
          position: 'bottom',
          labels: { color:'#374151', font:{ family:'Inter', weight:600 }, boxWidth:10, boxHeight:10, padding:10 }
        },
        tooltip: {
          backgroundColor:'#111827', titleColor:'#fff', bodyColor:'#fff',
          callbacks: { label: (ctx)=> ` ${ctx.label}: ${ctx.raw} turno(s)` }
        }
      }
    },
...
inBtn?.addEventListener('click', async ()=>{
  ...
  if(pin && pin === String(realPin)){ setAuthed(true); await showApp(); }
  else { Swal.fire({icon:'error', title:'PIN incorrecto', confirmButtonColor:'#ec4899'}); }
});
pinInp?.addEventListener('keydown', e=>{ if(e.key==='Enter') inBtn.click(); });
